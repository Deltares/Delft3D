...
class CgalConan(ConanFile):
    ...
    def generate(self):
        ...
        tc.variables["CGAL_CMAKE_EXACT_NT_BACKEND"] = "BOOST"
        tc.generate()
    ...
    def _create_cmake_module_variables(self, module_file):
        ...
        content += textwrap.dedent('''\
            ...
            target_compile_definitions(CGAL::CGAL INTERFACE CGAL_DISABLE_GMP=1)

            # CGAL use may rely on the presence of those two variables
            set(CGAL_USE_GMP  FALSE CACHE INTERNAL "CGAL library is configured without GMP")
            set(CGAL_USE_MPFR FALSE CACHE INTERNAL "CGAL library is configured without MPFR")
            set(CGAL_CMAKE_EXACT_NT_BACKEND BOOST CACHE INTERNAL "CGAL exact number type backend")
        ''')
