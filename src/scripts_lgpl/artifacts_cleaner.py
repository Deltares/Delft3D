...
def ossRemove():
    ...
    removeDirs = ["x64/menu", "x64/plugins", "lnx64/share/doc", "lnx64/share/man"]
    os.chdir(root)
...
def dimrsetRemove():
    ...
    removeDirs = ["lnx64/share/doc", "lnx64/share/man"]
    os.chdir(root)
    for path, dirs, files in os.walk(root):
        for aFile in files:
            name, extension = os.path.splitext(aFile)
...
def delft3d4Remove():
    ...
    removeDirs = ["lnx64/share/delft3d/esmf", "x64/menu", "x64/plugins", "lnx64/share/doc", "lnx64/share/man"]
    os.chdir(root)
    for path, dirs, files in os.walk(root):
        for aFile in files:
            name, extension = os.path.splitext(aFile)
...
parser.add_argument(
    "--product", help="Product to be cleaned (oss, dimrset, delft3d4).", default=None, required=True, dest="product"
)
parser.add_argument(
    "--root",
