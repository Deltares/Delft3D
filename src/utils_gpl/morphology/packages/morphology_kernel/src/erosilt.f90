...
!
   ...
   real(fp) :: slopefrac !> coefficient in linear regression (-)
   real(fp) :: tcrero_old !> tcrero, but old (N/m2)
   ...
   if (flmd2l) then
      ...
      if (iform == -3) then
         ...
         if (maxslope > wetslope) then
            ! Reduce the critical erosion shear stress once the local bed
            ! slope exceeds wetslope. The reduction is linear and reaches
            ! taucrmin at betaslope*wetslope.

            taucrmin = 0.1_fp
            betaslope = 2.0_fp

            if (wetslope > 0.0_fp .and. betaslope > 1.0_fp) then     ! safety when somebody decides to change beta
               slopefrac = (maxslope - wetslope) / ((betaslope - 1.0_fp) * wetslope)
               slopefrac = min(1.0_fp, slopefrac)

               tcrero_old = tcrero
               tcrero = tcrero_old + slopefrac * (taucrmin - tcrero_old)
            end if
         end if
         !
         if (sc_intrawave_method == SC_INTRAWAVE_LEGACY) then
