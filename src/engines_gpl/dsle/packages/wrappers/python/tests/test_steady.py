...
class TestSaltLoadSteady(unittest.TestCase):
    ...
    def test_deeper_lock(self):
        ...
        sl_low_bottom = dsle_calc_steady(
            **dict(
                self.parameters,
                lock_bottom=-8.0,
            )
        ...
        sl_higher_bottom = dsle_calc_steady(
            **dict(
                self.parameters,
                lock_bottom=-2.0,
            )
    ...
    def test_lock_dimensions(self):
        sl_lock_longer = dsle_calc_steady(
            **dict(
                self.parameters,
                lock_length=480.0,
            )
        ...
        sl_lock_wider = dsle_calc_steady(
            **dict(
                self.parameters,
                lock_width=24.0,
            )
    ...
    def test_num_cycles(self):
        ...
        for num_cycles, sl_ref in num_cycles_to_sl:
            ...
            self.assert_allclose_loose(result["salt_load_lake"], sl_ref, err_msg=f"num_cycles: {num_cycles}")

    def test_quicker_door_level(self):
        sl_door_quick_open = dsle_calc_steady(
            **dict(
                self.parameters,
                door_time_to_open=0.0,
            )
        )["salt_load_lake"]

        sl_door_quick_level = dsle_calc_steady(
            **dict(
                self.parameters,
                leveling_time=0.0,
            )
    ...
    def test_symmetry_coefficient(self):
        results_sym_0_5 = dsle_calc_steady(
            **dict(
                self.parameters,
                symmetry_coefficient=0.5,
            )
        )

        results_sym_1_5 = dsle_calc_steady(
            **dict(
                self.parameters,
                symmetry_coefficient=1.5,
            )
        )
        ...
        self.assert_allclose_tight(
            disch_to_lake_sym_0_5,
            disch_to_sea_sym_1_5,
        )
        self.assert_allclose_tight(
            disch_to_sea_sym_0_5,
            disch_to_lake_sym_1_5,
        )
    ...
    def test_ship_water_deplacement(self):
        results_ship_sea_to_lake = dsle_calc_steady(
            **dict(
                self.parameters,
                ship_volume_sea_to_lake=5000.0,
            )
        )
        ...
        results_ship_lake_to_sea = dsle_calc_steady(
            **dict(
                self.parameters,
                ship_volume_lake_to_sea=5000.0,
            )
        )
    ...
    def test_bubble_screen(self):
        sl_bubble_50 = dsle_calc_steady(
            **dict(
                self.parameters,
                density_current_factor_sea=0.5,
                density_current_factor_lake=0.5,
            )
        )["salt_load_lake"]
    ...
    def test_flushing_equal_head(self):
        results_flushing_lw = dsle_calc_steady(
            **dict(
                self.parameters,
                flushing_discharge_low_tide=1.0,
            )
        )
        ...
        results_flushing_hw = dsle_calc_steady(
            **dict(
                self.parameters,
                flushing_discharge_high_tide=1.0,
            )
        )
    ...
    def test_low_high_tide(self):
        results_low_tide = dsle_calc_steady(
            **dict(
                self.parameters,
                head_sea=-2.0,
            )
        ...
        results_high_tide = dsle_calc_steady(
            **dict(
                self.parameters,
                head_sea=2.0,
            )
        ...
        self.assertGreater(-results_high_tide["salt_load_lake"], -results_flushing_high_tide["salt_load_lake"])
        self.assertGreater(-results_low_tide["salt_load_lake"], -results_flushing_low_tide["salt_load_lake"])

        self.assertGreater(results_low_tide["discharge_from_lake"], self.reference_results["discharge_from_lake"])
        self.assertGreater(results_high_tide["discharge_from_sea"], self.reference_results["discharge_from_sea"])
    ...
    def test_sill(self):
        ...
        sl_sill_lake = dsle_calc_steady(
            **dict(
                self.parameters,
                sill_height_lake=1.0,
            )
        )["salt_load_lake"]
    ...
    def test_distance_door_bubble_screen(self):
        base_params = dict(self.parameters, density_current_factor_sea=0.25, density_current_factor_lake=0.25)

        sl_bubble_base = dsle_calc_steady(
            **dict(
                base_params,
            )
        )["salt_load_lake"]
        ...
        sl_bubble_distance_sea = dsle_calc_steady(
            **dict(
                base_params,
                distance_door_bubble_screen_sea=4.0,
            )
        )["salt_load_lake"]
        ...
        sl_bubble_distance_lake = dsle_calc_steady(
            **dict(
                base_params,
                distance_door_bubble_screen_lake=4.0,
            )
        )["salt_load_lake"]
