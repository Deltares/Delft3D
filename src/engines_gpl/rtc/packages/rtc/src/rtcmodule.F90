...
!  HarmonIt March 2004 - RTCNArgs vervangen door NrArgs
  ...
  551    CONTINUE
         ...
         ELSE
           ...
           CALL ERRMSG2 (999, IECODE, NamSub, ' Error in RTC command line', IOUT1)
           RetVal = 999
           Return
...
! *** Error messages
  ...
  555 CONTINUE
         CALL ERRMSG2 (902, IECODE, NAMSUB, STRING, IOUT1)
         RetVal = 902
!
   23 CONTINUE
         IF (I .GE. NFILE) THEN
...
!-------Check 'RUNRTC' by telephone
        if (icheck .eq. 786782) then
           ...
           CALL ERRMSG2 (946, 0, 'RTC', ' Trigger-file TMP_SYNC.RUN not made by Delft3D-FLOW ', IOUT1)
           RtcInitialize = 946
           Return
        endif
      else
...
! *********************************************************************
      ...
      If (RTCInitialize .eq. 981) then
          CALL ERRMSG2 (981, 0, ' Error allocating arrays in subroutine ', ' ReadLanguage ',Iout1)
      ElseIf (RTCInitialize .eq. 972) then
          Call ERRMSG2 (972, 0,'Error in language file',' ',IOUT1)
      Endif
      if (RtcInitialize .ne. 0) Return
      Close(IN)
...
!     RTC must be in SOBEK-Mode or in Delft3D-Mode
      If (.NOT. SOBEKMODE .AND. .NOT. CoupledToDelft3D) Then
         CALL ERRMSG2 (921, 0, ' RTC', ' RTC-Ini file', IOUT1)
         RTCInitialize = 921
         Return
      Endif
...
!  DIO: all communication set to HIS files, other options not supported anymore
      If (IOptS .ne. 2. .or. Iopt3 .ne. 2 .or. IoptWq .ne. 2) Then
         ...
         Iopt3 = 2
         IoptWq = 2
         CALL ERRMSG2 (953, 0, ' RTC', ' RTC-Ini file', IOUT1)
         RTCInitialize = 953
         Return
      Endif
...
!     Wind is only possible together wit External Locations
      If (USEW .AND. .NOT. USEEXT) Then
         CALL ERRMSG2 (944, 0, ' RTC', ' RTC-Ini file', IOUT1)
         RTCInitialize = 944
         Return
      Endif
...
!                                      energy flow demand time tables (turbine gates only)
      ...
      if (.not. success) then
         RtcInitialize = 929
         CALL ERRMSG2 (RtcInitialize, 0,' Allocating Table data',' NewTableConfAr',IOUT1)
         Return
      Endif
...
!     Alloceer arrays
      ...
      If (NPar3Rsvp .gt.0) then
         ...
         If (RtcInitialize .ne. 0) then
            CALL ERRMSG2 (RtcInitialize, 0,' Allocating Reservoir data',' ',IOUT1)
            Return
         Endif
      Endif
...
!     WRITE(*,*) ' Command line: ', CMDLINE(1:len_trim(CMDLINE))
      IF (SOBEKMODE) THEN
        ...
        IF (ISTAT .LT. 0)  THEN
            ...
            CALL ERRMSG2 (990, 0, ' RTC', STRING, IOUT1)
            RtcInitialize = 990
            Return
        ENDIF
...
! Check number of allocated possible datasets, at the moment linked to NParI=11
         if (NExtHd .gt. 2*NParI) then
            CALL ERRMSG2 (913, 0, 'Rtc',' NEXTHD datasets for external HIS files',IOUT1)
            RtcInitialize = 913
            If (RTCInitialize .ne. 0) Return
         Endif
...
! now put wind/precipitation, WQ off, and switch on in RdPara if they are used
      ...
      if (FNMEXT .OR. SOBEKMODE) then
        ...
        If (NPar3Rsvp .gt. 0) then
            ...
            If (RtcInitialize .ne. 0) then
               CALL ERRMSG2 (RtcInitialize, 0,' Reservoir data ',' ',IOUT1)
               Return
            Endif
            Close(In)
        Endif
...
!     Open MATLAB if MATUSE is TRUE
      ...
      If (RTCInitialize .ne. 0) then
         CALL ERRMSG2 (994, 0, ' MATLAB could not be started', ' ',Iout1)
         Return
      Endif
#endif
        ...
        If (CoupledToDelft3D .and. .not. SOBEKMODE) then
          ...
          If (lfout) then
            If (n2steps .eq. -1) then
              CALL ERRMSG2 (945, 0, ' SyncRtcFlow_Init', ' Shutdown from Flow', IOUT1)
              RTCInitializeEvent = 945
            Elseif (ND3D > 0 .and. .not. commFLOWtoRTC) then
              CALL ERRMSG2 (946, 0, ' SyncRtcFlow_Init', ' Delft3D-FLOW does not provide required quantities', IOUT1)
              RTCInitializeEvent = 946
            Elseif (commFLOWtoRTC .and. ND3D == 0) then
              CALL ERRMSG2 (946, 0, ' SyncRtcFlow_Init', ' Delft3D-FLOW provides data not required by RTC', IOUT1)
              RTCInitializeEvent = 946
            Else
              CALL ERRMSG2 (946, 0, ' SyncRtcFlow_Init', ' Init Error', IOUT1)
              RTCInitializeEvent = 946
            Endif
            Return
          Else
...
! End ARS 10827
          ...
          ELSE
             WRITE (*,*)  ITMSIZ, NRSRAI
             CALL ERRMSG2 (923, 0,'Predict',' ',IOUT1)
                ...
                Do i=1,MatlabNrWqPar
                  If (MatlabWqParId(i) .NE. '') Then
                     ...
                     If (MatlabWq(I) .GT. NPARQ .or. MatlabWQ (i) .le. 0) Then
                         ...
                         CALL ERRMSG2 (916, NPARQ, 'Rtc', ' Matlab WQ parameterfile', IOUT1)
...
! call external exe
           ...
           if (iret .ne. 0) CALL ERRMSG2 (RtcComputeTimestep, 0,' Error executing RunCommand TCN',' ComputeTimestep',IOUT1)
...
!get csv file with results (from TCN / external exe)
           IF (ReadCsvFile .ne. '') then
               ...
               !   CALL ERRMSG2 (RtcComputeTimestep, 0,' Error finding CSV file from TCN',' ComputeTimestep',IOUT1)
               endif
               CsvReadId = ''
               CsvReadValue = -999.
...
!                Read (IInCsv,*,Err=9981,End=999) CsvReadId(i), CsvReadValue(i)
 ...
 !               Write(*,*) i, CsvReadid(i), CsvReadValue(i)
                 goto 998
 9981            Continue
                 CALL ERRMSG2 (RtcComputeTimestep, 0,' Error reading CSV file from TCN',' ComputeTimestep',IOUT1)
  999            Continue
               Close(IInCsv)
               DO IPARA=12,NPARA
           ...
           If (Mod(Idum, OutputTimestep) .eq. 0) then
              ...
              if (NPara>0) then
                 ...
                 If (Allocation_Error .ne. 0) then
                    CALL ERRMSG2 (981, Allocation_Error, ' RTC', ' Error allocating arrays ', Iout1)
...
!!---------------------
   ...
   Function InitOutCFSetpoints(RTC_RunId) result(RetVal)
      ...
      If (OesInitialized() ) then
          ...
          If (.not. Success) then
              CALL ERRMSG2 (981, 0, ' Error alloc.openMI-arrs in subroutine ', &
