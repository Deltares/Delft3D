          ...
          if ( ((kcu(nm)==1) .and. (kfu(nm)==1)) .and. kspu0k /=4 .and. kspu0k /=10) then
             ...
             if (slplim) then
                ...
                if (s0(nm) < dpsmax) then
                   ...
                   !pressure   = - ag*rhofrac*(s0(nmu) - s_crit)/gvu(nm) !V:main
                   pressure = - ag*rhofrac*(s0(nmu) - dpsmax)/dksi !V: branch
                   densforce  = 0.0_fp
                elseif (s0(nmu) < dpsmax) then
                   s_crit     = twothird * max(0.0_fp, s0(nm)-dpsmax) + dpsmax
                   !pressure   = - ag*rhofrac*(s_crit  - s0(nm))/gvu(nm) !V: main
                   pressure = - ag*rhofrac*(dpsmax  - s0(nm))/dksi !V: branch
                   densforce  = 0.0_fp
                else
