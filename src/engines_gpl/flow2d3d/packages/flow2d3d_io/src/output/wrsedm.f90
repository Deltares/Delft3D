    ...
    case (REQUESTTYPE_WRITE)
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sbcu(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sbcv(nm,l)/ gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sbcuu(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sbcvv(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sbwu(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sbwv(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sbwuu(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sbwvv(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sswu(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sswv(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sswuu(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsedtot
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sswvv(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
       ...
       if (lsedtot > 0 .and. moroutput%sbuuvv) then
          ...
          do l = 1, lsedtot
             do m = 1, mmax
                do n = 1, nmaxus
                   rbuff3(n, m, l) = sbuu(n, m, l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                enddo
             enddo
          enddo
          call wrtarray_nml(fds, filename, filetype, grpnam, celidt, &
          ...
          do l = 1, lsedtot
             do m = 1, mmax
                do n = 1, nmaxus
                   rbuff3(n, m, l) = sbvv(n, m, l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                enddo
             enddo
          enddo
          call wrtarray_nml(fds, filename, filetype, grpnam, celidt, &
          ...
          if (associated(sbcu)) then
             do l = 1, lsed
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = ssuu(nm, l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (associated(sbcu)) then
             do l = 1, lsed
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = ssvv(nm, l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
          endif
          ...
          if (moroutput%suvcor) then
             ...
             do l = 1, lsed
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = sucor(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
             call wrtarray_nml(fds, filename, filetype, grpnam, celidt, &
             ...
             do l = 1, lsed
                do m = 1, mmax
                   do n = 1, nmaxus
                      call n_and_m_to_nm(n, m, nm, gdp)
                      rbuff3(n, m, l) = svcor(nm,l) / gdp%gdsedpar%sedtrans_unitcov_fac(l)
                   enddo
                enddo
             enddo
             call wrtarray_nml(fds, filename, filetype, grpnam, celidt, &
