...
!     kind           function         name                 description
    ...
    integer(kind=int_32)      , intent(in   ) ::lunin             !< unit number input file
    character*(*), intent(in   ) :: lfile             !< file name
    integer(kind=int_32)      , intent(in   ) ::lunut             !< unit number report file
    integer(kind=int_32)      , intent(in   ) ::npos              !< number of significant positions in one line
    real(kind=sp)         , intent(  out) ::input_version_number            !< Version number
    integer(kind=int_32)      , intent(  out) ::ioutpt            !< Output option

!     Local

    character*(npos) car                              !  read buffer
    character*1      ctrlz , ch_cr                    !  special characters
    integer(kind=int_32)           ::i, i2                            !  loop counter
    integer(kind=int_32)           ::status                           !  iostatus
...
!     kind           function         name                 description
      real(kind=sp)         , intent(in) ::input_version_number              ! Version number
      real(kind=sp)                       ::lower_limit_version ! lower limit of version numbers
      integer(kind=int_32)      , intent(in) ::lunut               ! unit number report file
