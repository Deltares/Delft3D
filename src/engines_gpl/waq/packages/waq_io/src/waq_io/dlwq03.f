...
!     kind           function         name                Descriptipon
      ...
      integer(kind=int_32), intent(inout) ::  nrharm (*)         !< number of harmonic items
      integer(kind=int_32), intent(  out) ::  ivflag             !< computed volumes ?
      ...
      integer(kind=int_32), dimension(3), intent(out)  ::  nexch  !< number of exchanges as read via hyd-file
      type(GridPointerColl)           GridPs            !< Collection of grid pointers

!     local decalations

      character*255           cdummy            !  workspace to read a string
      integer(kind=int_32) :: idummy             !  location to read an integer
      logical                 disper            !  is opt0 called for dispersions ?
      integer(kind=int_32) :: volume             !  is 1 if opt0 is called for volumes ?
      integer(kind=int_32) :: ifact              !  needed for call to opt0
      integer(kind=int_32) :: itype              !  type of token that is returned
      integer(kind=int_32) :: ierr2              !  local error indicator
      integer(kind=int_32) :: iwar2              !  local warning indicator
      integer(kind=int_32), allocatable ::  ikmerge(:)         !  array with indicators whether attributes are already set
      integer(kind=int_32), allocatable ::  iamerge(:)         !  composite attribute array
      integer(kind=int_32), allocatable ::  ikenm  (:)         !  array with attributes of an input block
      integer(kind=int_32), allocatable ::  iread  (:)         !  array to read attributes
      integer(kind=int_32), allocatable ::  pgrid  (:,:)       !  workspace for matrix with printed grids
      integer(kind=int_32) :: imopt1             !  first option for grid layout
      integer(kind=int_32) :: i, j, k            !  loop counters
      integer(kind=int_32) :: ikerr              !  error indicator attributes
      integer(kind=int_32) :: nkopt              !  number of attribute blocks
      integer(kind=int_32) :: nopt               !  that many attributes in this block
      integer(kind=int_32) :: ikopt1             !  first (file) option attributes
      integer(kind=int_32) :: ikopt2             !  second (default/nondefault) option attributes
      integer(kind=int_32) :: ikdef              !  default value attributes
      integer(kind=int_32) :: nover              !  number of overridings
      integer(kind=int_32) :: iover              !  overriding cell number
      integer(kind=int_32) :: iseg               !  loop counter computational volumes
      integer(kind=int_32) :: iknm1, iknm2       !  help variables for attributes
      integer(kind=int_32) :: iknmrk             !  help variables merged attributes
      integer(kind=int_32) :: ivalk              !  return value dhknmrk
      ...
      integer(kind=int_32) ::  ncid, ncidout 
      integer(kind=int_32) ::  varid, varidout, meshid, timeid, bndtimeid, ntimeid, wqid 
      integer(kind=int_32) ::  meshid2d, type_ugrid, meshid1d, networkid, network_geometryid 
...
!       Output formats
 ...
 2599 format ( / ' NetCDF error message: ', A )
      ...
      integer(kind=int_32), intent(in) ::  lunut       !< LU-number of the report file
      ...
      integer(kind=int_32), intent(out) ::  ierr2       !< Whether an error was found or not

      integer(kind=int_32) ::  i, ierr 
      integer(kind=int_32) ::  luvol 
      integer(kind=int_32) ::  time1, time2, time3 
      real(kind=sp) ::  dummy 
