        ...
        segmentLoop : do iseg = 1, num_cells
            ...
            if (ikmrk1==1) then
                ...
                speciesLoop : do iSpec = 1, nrSpec
                    ...
                    if (ieee_is_nan(protC)) call write_warning( 'ERROR: in ProtistDiat, NaN in protC in segment:', iseg )
                    if (ieee_is_nan(Cfix))  call write_warning( 'ERROR: in ProtistDiat, NaN in Cfix in segment:', iseg )
                    if (ieee_is_nan(totR))  call write_warning( 'ERROR: in ProtistDiat, NaN in totR in segment:', iseg )
                    if (ieee_is_nan(mrt))   call write_warning( 'ERROR: in ProtistDiat, NaN in mrt in segment:', iseg )
                    if (ieee_is_nan(NC))    call write_warning( 'ERROR: in ProtistDiat, NaN in NC in segment:', iseg )
                    if (ieee_is_nan(PC))    call write_warning( 'ERROR: in ProtistDiat, NaN in PC in segment:', iseg )
                    if (ieee_is_nan(ChlC))  call write_warning( 'ERROR: in ProtistDiat, NaN in ChlC in segment:', iseg )
                    if (ieee_is_nan(SC))    call write_warning( 'ERROR: in ProtistDiat, NaN in SC in segment:', iseg )
