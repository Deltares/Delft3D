        ...
        segmentLoop : do iseg = 1, num_cells
            ...
            if (ikmrk1==1) then
                ...
                speciesLoop : do iSpec = 1, nrSpec
                    ...
                    if (ieee_is_nan(protC)) call write_warning( 'ERROR: in ProtistZoo, NaN in protC in segment:', iseg )
                    if (ieee_is_nan(ingC))  call write_warning( 'ERROR: in ProtistZoo, NaN in ingC in segment:', iseg )
                    if (ieee_is_nan(assC))  call write_warning( 'ERROR: in ProtistZoo, NaN in assC in segment:', iseg )
                    if (ieee_is_nan(totR))  call write_warning( 'ERROR: in ProtistZoo, NaN in totR in segment:', iseg )
                    if (ieee_is_nan(mrt))   call write_warning( 'ERROR: in ProtistZoo, NaN in mrt in segment:', iseg )
