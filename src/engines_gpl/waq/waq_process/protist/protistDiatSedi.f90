   ...
   subroutine PROSED(process_space_real, fl, ipoint, increm, num_cells, &
      ...
      do iseg = 1, num_cells
         ...
         if (ikmrk1 == 1) then
            ...
            end if ! end if check for dry cell
            ...
            if (ikmrkv == 1 .and. ikmrkn == 1) then
               ...
               do iSpec = 1, nrSpec
                  ...
                  if (Depth > MinDepth .and. Depth2 > MinDepth2) then
                     ...
                     process_space_real(ipnt(inpItems + nrSpec + nrSpec * nrSpecOut + iSpec)) = 0.0
                  end if ! end check if min depth large enough

               end do ! end loop over species
            end if ! end check water - water
         end if ! end check boundaries
         ipnt(1:ipointLength) = ipnt(1:ipointLength) + increm(1:ipointLength)
      end do ! end loop of vertical exchange
      deallocate (ipnt)
      return
   end
   ! end subroutine
