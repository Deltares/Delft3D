...
!$f90tw TESTCODE(TEST, test_coordinate_transform, test_tangential_velocity_equivalence, test_tangential_velocity_equivalence,
   subroutine test_tangential_velocity_equivalence() bind(C)
      ...
      ! Step 1: ux3, uy3 contain ucxq values (from prefetch_node_velocities)
      ! Step 2: Transform to link frame
      !   ucxq_link_1 = csb_1(L) * ux3(L) + snb_1(L) * uy3(L)  ! link-x component
      !   ucyq_link_1 = -snb_1(L) * ux3(L) + csb_1(L) * uy3(L) ! link-y component
      ! Step 3: Compute tangential
      !   tangential = -snu(L) * ucxq_link_1 + csu(L) * ucyq_link_1
      
      tangential_refactor_node1 = -snu(L) * (csb_1(L) * ux3(L) + snb_1(L) * uy3(L)) + &
                                   csu(L) * (-snb_1(L) * ux3(L) + csb_1(L) * uy3(L))
      
      tangential_refactor_node2 = -snu(L) * (csb_2(L) * ux4(L) + snb_2(L) * uy4(L)) + &
                                   csu(L) * (-snb_2(L) * ux4(L) + csb_2(L) * uy4(L))
      
      ! THE CRITICAL TEST: These should match!
