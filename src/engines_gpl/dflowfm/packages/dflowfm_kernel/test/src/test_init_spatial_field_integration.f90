...
contains
   ...
   subroutine test_rainfall_bcascii_registers_ec_connection() bind(C)
      ...
      call create_file(BC_FILENAME, [character(len=512) :: &
                       "[General]", &
                       "    fileVersion           = 1.01", &
                       "    fileType              = boundConds", &
                       "", &
      ...
      call create_file(EXT_FILENAME, [character(len=512) :: &
                       "[Spatial]", &
                       "    quantity        = rainfall", &
   ...
   subroutine test_solarradiation_conflicts_with_netsolarradiation() bind(C)
      ...
      call create_file(SOLAR_BC, [character(len=512) :: &
                       "[General]", &
                       "    fileVersion           = 1.01", &
                       "    fileType              = boundConds", &
                       "", &
      ...
      call create_file(SOLAR_EXT, [character(len=512) :: &
                       "[Spatial]", &
   ...
   subroutine test_qext_static_field_populated_at_init() bind(C)
      ...
      ! ARRANGE: one sample point exactly at the single grid cell (0,0) with value 1.5.
            call create_file(SAMPLE_FILE, [character(len=512) :: "-1.0 -1.0  1.5", &
                                      " 1.0 -1.0  1.5", &
                                      " 0.0  1.0  1.5"])

      call create_file(QEXT_EXT, [character(len=512) :: &
                       "[Spatial]", &
                       "    quantity        = qext", &
   ...
   subroutine test_qext_bcascii_registers_ec_connection() bind(C)
      ...
      call create_file(QEXT_BC, [character(len=512) :: &
                       "[General]", &
                       "    fileVersion = 1.01", &
                       "    fileType    = boundConds", &
                       "", &
   ...
   subroutine test_initialwaterlevel_static_field_populated_at_init() bind(C)
      ...
      call create_file(SAMPLE_FILE, [character(len=512) :: "-1.0 -1.0  1.5", &
                                     " 1.0 -1.0  1.5", &
                                     " 0.0  1.0  1.5"])
      call create_file(EXT_FILE, [character(len=512) :: &
                       "[Spatial]", &
   ...
   subroutine test_frictioncoefficient_static_field_populated_at_init() bind(C)
      ...
      call create_file(SAMPLE_FILE, [character(len=512) :: "-1.0 -1.0  0.02", &
                                     " 1.0 -1.0  0.02", &
                                     " 0.0  1.0  0.02"])
      call create_file(EXT_FILE, [character(len=512) :: &
                       "[Spatial]", &
   ...
   subroutine test_initialwaterdepth_derives_s1() bind(C)
      ...
      call create_file(SAMPLE_FILE, [character(len=512) :: "-1.0 -1.0  2.0", &
                                     " 1.0 -1.0  2.0", &
                                     " 0.0  1.0  2.0"])
      call create_file(EXT_FILE, [character(len=512) :: &
                       "[Spatial]", &
   ...
   subroutine test_secchidepth_static_field_and_post_processing() bind(C)
      ...
      call create_file(SAMPLE_FILE, [character(len=512) :: "-1.0 -1.0  3.5", &
                                     " 1.0 -1.0  3.5", &
                                     " 0.0  1.0  3.5"])
      call create_file(EXT_FILE, [character(len=512) :: &
                       "[Spatial]", &
   ...
   subroutine test_advectiontype_integer_field_populated() bind(C)
      ...
      call create_file(POL_FILE, [character(len=512) :: &
                       "enclosing_polygon", &
                       "5  2", &
                       "-2.0  -2.0", &
                       " 2.0  -2.0", &
   ...
   subroutine test_initialverticalsalinityprofile() bind(C)
      ...
      call create_file(PROFILE_FILE, [character(len=512) :: &
                       "salinityprofile", &
                       "2  2", &
                       "-10.0  35.0", &
                       "  0.0  35.0"])
   ...
   subroutine test_field1d_global_value_applied_to_frictioncoefficient() bind(C)
      ...
      call create_file(FIELD1D_FILE, [character(len=512) :: &
                       "[General]", &
                       "    fileVersion = 1.00", &
                       "    fileType    = 1dField", &
                       "", &
