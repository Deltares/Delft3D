...
contains
   ...
   subroutine test_compute_scales_and_fluxes_moninobukhov_true() bind(C)
      ...
      call f90_expect_near(u_star(1), 0.483565544130436_dp, 1e-5_dp, "u_star(1) does not match expected value.")
      call f90_expect_near(t_star(1), -0.017773821047599_dp, 1e-7_dp, "t_star(1) does not match expected value.")
      call f90_expect_near(q_star(1), -0.000070993355341_dp, 1e-9_dp, "q_star(1) does not match expected value.")
      call f90_expect_near(z0_momentum(1), 0.000765992034486_dp, 1e-7_dp, "z0_momentum(1) does not match expected value.")
      call f90_expect_near(z0_heat(1), 0.000012408508964_dp, 1e-9_dp, "z0_heat(1) does not match expected value.")
      call f90_expect_near(z0_humidity(1), 0.000019233188894_dp, 1e-9_dp, "z0_humidity(1) does not match expected value.")
      call f90_expect_near(monin_obukhov_length(1), -574.377132275664508_dp, 1e-1_dp, "monin_obukhov_length(1) does not match expected value.")
      call f90_expect_near(richardson_number(1), -0.002647192937021_dp, 1e-7_dp, "richardson_number(1) does not match expected value.")
      call f90_expect_true(comparereal(wind_stress_u(1), 0.1982699171592206_dp) == 0, "wind_stress_u(1) does not match expected value")
      call f90_expect_true(comparereal(wind_stress_v(1), 0.20705539138130558_dp) == 0, "wind_stress_v(1) does not match expected value")
      call f90_expect_true(comparereal(latent_heat_flux(1), -105.25305258424659_dp) == 0, "latent_heat_flux(1) does not match expected value")
      call f90_expect_true(comparereal(sensible_heat_flux(1), -9.0742803933853988_dp) == 0, "sensible_heat_flux(1) does not match expected value")
      
      call f90_expect_near(u_star(2), 0.021984244610906_dp, 1e-5_dp, "u_star(2) does not match expected value.")
      call f90_expect_near(t_star(2), -0.063784561633405_dp, 1e-5_dp, "t_star(2) does not match expected value.")
      call f90_expect_near(q_star(2), -0.000148948329343_dp, 1e-8_dp, "q_star(2) does not match expected value.")
      call f90_expect_near(z0_momentum(2), 0.000075458070599_dp, 1e-8_dp, "z0_momentum(2) does not match expected value.")
      call f90_expect_near(z0_heat(2), 0.000272980043536_dp, 1e-7_dp, "z0_heat(2) does not match expected value.")
      call f90_expect_near(z0_humidity(2), 0.000423119067481_dp, 1e-6_dp, "z0_humidity(2) does not match expected value.")
      call f90_expect_near(monin_obukhov_length(2), -0.401481304017752_dp, 1e-4_dp, "monin_obukhov_length(2) does not match expected value.")
      call f90_expect_near(richardson_number(2), -1.974463098626122_dp, 1e-4_dp, "richardson_number(2) does not match expected value.")
      call f90_expect_true(comparereal(wind_stress_u(2), 0.00014672413485498765_dp) == 0, "wind_stress_u(2) does not match expected value")
      call f90_expect_true(comparereal(wind_stress_v(2), -0.00056732363949399942_dp) == 0, "wind_stress_v(2) does not match expected value")
      call f90_expect_true(comparereal(latent_heat_flux(2), -9.9286200776872224_dp) == 0, "latent_heat_flux(2) does not match expected value")
      call f90_expect_true(comparereal(sensible_heat_flux(2), -1.4641448504870007_dp) == 0, "sensible_heat_flux(2) does not match expected value")
