...
!> Writes the unstructured network in UGRID format to an already opened netCDF dataset.
   subroutine unc_write_net_ugrid2(ncid, id_tsp, janetcell, jaidomain, jaiglobal_s)
      ...
      if (numl2d > 0) then ! 2D net geometry
         ...
         if (n1d2dcontacts > 0) then
            ierr = ug_def_mesh_contact(ncid, id_tsp%meshcontacts_1D2D, trim(contactname)//'1D2D', n1d2dcontacts, id_tsp%meshids1d, id_tsp%meshids2d, UG_LOC_NODE, UG_LOC_FACE, start_index)
            ierr = nf90_enddef(ncid)
            ! Put the contacts
         ...
         if (n2d2dcontacts > 0) then
            ierr = ug_def_mesh_contact(ncid, id_tsp%meshcontacts_2D2D, trim(contactname)//'2D2D', n2d2dcontacts, id_tsp%meshids2d, id_tsp%meshids2d, UG_LOC_FACE, UG_LOC_FACE, start_index)
            ierr = nf90_enddef(ncid)
            ! Put the contacts
...
!   ierr = unc_def_var_map(mapids, mapids%id_flowelemcontourx(:), nf90_double, UNC_LOC_S, 'FlowElemContour_x', '', '', 'm', [ id_flowelemcontourptsdim, id_seddim, -1 ]).
      ...
      if (n1d2dcontacts > 0 .and. ja2D_) then
         ierr = ug_def_mesh_contact(ncid, id_tsp%meshcontacts_1D2D, trim(contactname), n1d2dcontacts, id_tsp%meshids1d, id_tsp%meshids2d, UG_LOC_NODE, UG_LOC_FACE, start_index)
      end if
