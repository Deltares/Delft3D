...
contains
   ...
   subroutine unc_write_his(tim) ! wrihis
      ...
      if (write_his_output%zcor > 0) then
         jawrizc = 1
         jawrizw = 1
      end if
      ...
      if (.not. model_has_obs_stations() .and. ncrs <= 0 .and. write_his_output%bal <= 0 .and. write_his_output%cgen <= 0 .and. num_rugs <= 0) then
         if (ihisfile == 0) then
      ...
      if (ihisfile == 0) then
         ...
         if ((kmx > 0) .or. (write_his_output%zcor > 0)) then
         ...
         if (stm_included .and. write_his_output%sed > 0) then
            if (ISED1 > 0) then
         ...
         ! Source-sinks
         if (write_his_output%sourcesink > 0 .and. num_source_sink > 0) then
         ...
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_SOURCE_SINK, write_his_output%sourcesink, num_source_sink, 'line', nNodeTot, id_strlendim, &
         ...
         if (write_his_output%cgen > 0 .and. ngenstru_ > 0) then
            nNodeTot = 0
         ...
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_GENERAL_ST, write_his_output%cgen, ngenstru_, 'line', nNodeTot, id_strlendim, &
         ...
         ! Pump
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_PUMP, write_his_output%pump, npumpsg, 'line', number_of_pump_nodes(), id_strlendim, &
         ...
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_GATE, write_his_output%gate, ngatesg, 'none', 0, id_strlendim, &
                                                  id_gatedim, id_gate_id)
         ...
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_GATEGEN, write_his_output%gate, ngategen, 'line', nNodeTot, id_strlendim, &
         ...
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_DAM, write_his_output%cdam, ncdamsg, 'none', 0, id_strlendim, &
                                                  id_cdamdim, id_cdam_id)

         ! Weir
         if (write_his_output%weir > 0 .and. nweirgen > 0) then
            ! Define geometry related variables
            nNodeTot = 0
         ...
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_WEIR, write_his_output%weir, nweirgen, 'line', nNodeTot, id_strlendim, &
         ...
         ! Orifice
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_ORIFICE, write_his_output%orifice, network%sts%numOrifices, 'line', nNodesOrif, id_strlendim, &
                                                  id_orifgendim, id_orifgen_id, id_orifgengeom_node_count, id_orifgengeom_node_coordx, id_orifgengeom_node_coordy, &
                                                  id_poly_xmid=id_weirgen_xmid, id_poly_ymid=id_weirgen_ymid)

         ! Bridge
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_BRIDGE, write_his_output%bridge, network%sts%numBridges, 'line', nNodesBridge, id_strlendim, &
         ...
         ! Culvert
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_CULVERT, write_his_output%culvert, network%sts%numculverts, 'line', nNodesCulv, id_strlendim, &
         ...
         ! Dambreak
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_DAMBREAK, write_his_output%dambreak, n_db_signals, 'none', 0, id_strlendim, &
                                                  id_dambreakdim, id_dambreak_id)

         ! Universal weir
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_UNI_WEIR, write_his_output%universal_weir, network%sts%numuniweirs, 'line', nNodesUniweir, id_strlendim, &
         ...
         ! compound structure
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_COMPOUND, write_his_output%compound_structure, network%cmps%count, 'none', 0, id_strlendim, &
                                                  id_cmpstrudim, id_cmpstru_id)

         ! Long culvert
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_LONGCULVERT, write_his_output%long_culvert, nlongculverts, 'line', nNodesLongCulv, id_strlendim, &
         ...
         ! Lateral
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_LATERAL, write_his_output%lateral, numlatsg, 'point', nNodesLat, id_strlendim, &
         ...
         if (write_his_output%bedlev > 0 .and. model_has_obs_stations() .and. .not. stm_included) then
         ...
         if (it_his == 0) then
            ...
            ! Lateral discharges
            if (write_his_output%lateral > 0 .and. numlatsg > 0) then
            ...
            if (jased > 0 .and. stm_included .and. write_his_output%sed > 0 .and. ISED1 > 0) then
...
!   Observation points (fixed+moving)
      ...
      if (ntot > 0 .and. .not. stm_included .and. write_his_output%bedlev > 0) then
   ...
   subroutine unc_put_his_structure_static_vars(ncid)
      ...
      call unc_put_his_structure_names(ncid, write_his_output%weir, id_weirgen_id, structure_names)
      ...
      call unc_put_his_structure_names(ncid, write_his_output%orifice, id_orifgen_id, structure_names)

      structure_names = [(pump_ids(i), integer :: i=1, npumpsg)]
      call unc_put_his_structure_names(ncid, write_his_output%pump, id_pump_id, structure_names)
      call unc_put_his_structure_mid_points(ncid, ST_PUMP, write_his_output%pump, npumpsg, 'line', id_poly_xmid=id_pump_xmid, id_poly_ymid=id_pump_ymid)

      structure_names = [(gate_ids(i), integer :: i=1, ngatesg)]
      call unc_put_his_structure_names(ncid, write_his_output%gate, id_gate_id, structure_names)
      ...
      call unc_put_his_structure_names(ncid, write_his_output%cgen, id_genstru_id, structure_names)
      ...
      call unc_put_his_structure_names(ncid, write_his_output%universal_weir, id_uniweir_id, structure_names)

      structure_names = get_dambreak_names()
      call unc_put_his_structure_names(ncid, write_his_output%dambreak, id_dambreak_id, structure_names)
      ...
      call unc_put_his_structure_names(ncid, write_his_output%culvert, id_culvert_id, structure_names)
      ...
      call unc_put_his_structure_names(ncid, write_his_output%bridge, id_bridge_id, structure_names)

      structure_names = [(network%cmps%compound(i)%id, integer :: i=1, network%cmps%count)]
      call unc_put_his_structure_names(ncid, write_his_output%compound_structure, id_cmpstru_id, structure_names)

      structure_names = [(longculverts(i)%id, integer :: i=1, nlongculverts)]
      call unc_put_his_structure_names(ncid, write_his_output%long_culvert, id_longculvert_id, structure_names)

      structure_names = [(cdam_ids(i), integer :: i=1, ncdamsg)]
      call unc_put_his_structure_names(ncid, write_his_output%cdam, id_cdam_id, structure_names)
      ...
      call unc_put_his_structure_names(ncid, write_his_output%sourcesink, id_srcname, structure_names)
      ...
      call unc_put_his_structure_names(ncid, write_his_output%gate, id_gategen_id, structure_names)

      structure_names = [(lat_ids(i), integer :: i=1, numlatsg)]
      call unc_put_his_structure_names(ncid, write_his_output%lateral, id_lat_id, structure_names)
