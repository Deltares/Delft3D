...
contains
   ...
   subroutine unc_write_his(tim) ! wrihis
      ...
      use unstruc_channel_flow, only: network
      use m_dambreak_data, only: p_n_db_links, p_n_db_signals
      ...
      if (ihisfile == 0) then
         ...
         maxlocT = max(size(valobs, 2), npumpsg, network%sts%numPumps, ngatesg, ncdamsg, ncgensg, ngategen, &
                       ...
                       p_n_db_links, network%sts%numOrifices, network%sts%numBridges, network%sts%numculverts, &
         ...
         ! Dambreak
         ierr = unc_def_his_structure_static_vars(ihisfile, ST_DAMBREAK, jahisdambreak, p_n_db_signals, 'none', 0, id_strlendim, &
                                                  id_dambreakdim, id_dambreak_id)
...
!   Observation points (fixed+moving)
   ...
   subroutine unc_put_his_structure_static_vars(ncid)
      ...
      use m_dambreak_data, only: db_ids, p_n_db_signals
      use unstruc_channel_flow, only: network
      ...
      structure_names = [(db_ids(i), integer :: i=1, p_n_db_signals)]
