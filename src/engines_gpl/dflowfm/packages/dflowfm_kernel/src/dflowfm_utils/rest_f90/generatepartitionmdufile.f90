      ...
      do while (stat == 0)
         ...
         elseif (mdu_line_main(1:1) == '[' .and. in_numerics) then ! About to close [numerics] block
            if (.not. icgsolver_present) then
               ...
               mdu_line_partition = "Icgsolver = "//trim(adjustl(string_icgsolver))//"          # Solver type , 1 = sobekGS_OMP, 2 = sobekGS_OMPthreadsafe, 3 = sobekGS, 4 = sobekGS + Saadilud, 6 = parallel/PETSc"
         ...
         if (.not. any(keyword_present)) then
            ...
            if (keyword_present(1)) then ! Modify NetFile
               ...
               write (string_icgsolver, "(I5)") md_icgsolver
               mdu_line_partition = trim(keyword)//" "//trim(adjustl(string_icgsolver))//"          # Solver type , 1 = sobekGS_OMP, 2 = sobekGS_OMPthreadsafe, 3 = sobekGS, 4 = sobekGS + Saadilud, 6 = parallel/PETSc"
               write (unit_partition, "(a)") trim(mdu_line_partition)
