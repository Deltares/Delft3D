...
contains
   ...
   subroutine flow_init_statistical_output_his(output_config_set, output_set)
      ...
      !
      if (write_his_output%bal > 0) then
      ...
      !
      if (write_his_output%cgen > 0 .and. ngenstru > 0) then
      ...
      if (write_his_output%gate > 0 .and. ngatesg > 0) then
      ...
      if (write_his_output%gate > 0 .and. ngategen > 0) then
      ...
      if (model_has_obs_stations()) then
         ...
         if (write_his_output%discharge > 0) then
            if (model_is_3D()) then
         ...
         ! Gravity + buoyancy
         if (jasal > 0 .and. write_his_output%sal > 0) then
            if (model_is_3D()) then
         ...
         if (temperature_model /= TEMPERATURE_MODEL_NONE .and. write_his_output%tem > 0) then
            if (model_is_3D()) then
         ...
         ! Wave model
         if (jawave > NO_WAVES .and. write_his_output%wav > 0) then
         ...
         ! Meteo
         if (air_pressure_available .and. write_his_output%wind > 0) then
            call add_stat_output_items(output_set, output_config_set%configs(IDX_HIS_PATM), valobs(:, IPNT_PATM))
         end if

         if (jawind > 0 .and. write_his_output%wind > 0) then
            if (jsferic == 0) then
         ...
         if (ja_airdensity + ja_computed_airdensity > 0 .and. write_his_output%airdensity > 0) then
         ...
         if (temperature_model == TEMPERATURE_MODEL_EXCESS .or. temperature_model == TEMPERATURE_MODEL_COMPOSITE) then
            if (write_his_output%heatflux > 0) then
         ...
         else if (stm_included .and. ISED1 > 0 .and. write_his_output%sed > 0 .and. IVAL_SF1 > 0) then
            if (model_is_3D()) then
         ...
         if (write_his_output%sed > 0 .and. jased > 0 .and. stm_included) then
