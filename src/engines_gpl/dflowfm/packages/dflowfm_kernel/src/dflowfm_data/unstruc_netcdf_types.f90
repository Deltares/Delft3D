...
module m_unstruc_netcdf_types
   ...
   use meshdata, only: t_ug_meshgeom_destructor

   implicit none(type, external)

!> D-Flow FM-specific flow geometry object containing the 1D and 2D UGRID-compliant mesh data, as well as D-Flow FM-specific administration.
...
!! global numbering in D-Flow FM to the 1D/2D separate (possibly mask-reduced) UGRID numbering.
   type t_fm_flowgeom
      ...
      integer, dimension(:), allocatable :: edge_map_2D !< 2D: mapping from reduced output set UGRID edge index to local net link index.
      integer, dimension(:), allocatable :: node_map_2D !< 2D: mapping from reduced output set UGRID node index to global net node number.
      ...
      integer, dimension(:), allocatable :: contacttype !< 1D2D contact link code, see kn(3,:) in network_data.f90.
      ...
      integer :: ndx_out = 0 !< Total number of output nodes.
...
contains
   ...
   subroutine reset_fm_flowgeom(flowgeom)
      type(t_fm_flowgeom), intent(inout) :: flowgeom

      integer :: ierr

      ierr = t_ug_meshgeom_destructor(flowgeom%mesh1D)
      ierr = t_ug_meshgeom_destructor(flowgeom%mesh2D)
      flowgeom = t_fm_flowgeom()
   end subroutine reset_fm_flowgeom
