...
contains
   ...
   subroutine alloc_transport(Keepexisting)
      ...
      use fm_external_forcings_data, only: num_source_sink, qcsrc, vcsrc, wstracers
...
!  tracer boundary condition
      ...
      call realloc(qcsrc, [NUMCONST, num_source_sink], keepExisting=.false., fill=0.0_dp)
      call realloc(vcsrc, [2 * NUMCONST, num_source_sink], keepExisting=.false., fill=0.0_dp)
