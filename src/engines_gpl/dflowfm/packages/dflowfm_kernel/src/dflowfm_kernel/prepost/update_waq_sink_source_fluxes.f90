...
!!
!! The code uses similar ways to distribute discharges over layers as the calling subroutine asource_sinks%update_discharges. Changes in the
         ...
         if (source_sinks%waq_index(isrc) >= 0) then
            ...
            if (waqpar%kmxnxa == 1) then
               ...
               if (kksin == 0 .and. kksor /= 0) then
                  ...
                  do k = kbsor, ktsor
                     if (dzss > epshs) then
                        qsrck = source_sinks%discharge(isrc) * (zws(k) - zws(k - 1)) / dzss
                     else
