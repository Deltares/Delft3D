...
contains
   ...
   subroutine flow_allocflow()
      ...
      use m_flowtimes, only: dtcell, time_wetground, autotimestep, ja_timestep_nostruct, ti_waq
      ...
      if (kmx > 0 .and. (autotimestep == 3 .or. autotimestep == 4)) then
         call realloc(squ2D, ndkx, stat=ierr, fill=0.0_dp, keepexisting=.false.)
         call aerr('squ2D(ndkx)', ierr, ndkx)
      end if

      if (autotimestep == 1 .and. ja_timestep_nostruct > 0) then
