...
contains
   ...
   subroutine flow_allocflow()
      ...
      use m_flowtimes, only: dtcell, time_wetground, autotimestep, AUTOTS_2D_OUT, AUTOTS_3D_HOR_OUT, AUTOTS_3D_HOR_INOUT, ja_timestep_nostruct, ti_waq
      ...
      if (kmx > 0 .and. (autotimestep == AUTOTS_3D_HOR_OUT .or. autotimestep == AUTOTS_3D_HOR_INOUT)) then
         call realloc(squ2D, ndkx, stat=ierr, fill=0.0_dp, keepexisting=.false.)
         call aerr('squ2D(ndkx)', ierr, ndkx)
      end if

      if (autotimestep == AUTOTS_2D_OUT .and. ja_timestep_nostruct > 0) then
