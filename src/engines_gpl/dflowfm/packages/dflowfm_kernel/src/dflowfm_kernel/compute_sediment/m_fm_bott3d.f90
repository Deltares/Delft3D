...
contains
   ...
   subroutine fm_adjust_suspended_bedslope()
      ...
      use m_flowgeom, only: acl, csu, snu, wcx1, wcx2, wcy1, wcy2, wu_mor
      use m_flow, only: hu
      ...
      do lsus = 1, lsed
         ...
         do lf = 1, lnx
            ...
            dqc = suspended_slope_normal_flux(qn, qt, slopen, slopet)                           ! suspended, current
            dqw = suspended_slope_normal_flux(e_sswn(lf, l), e_sswt(lf, l), slopen, slopet)     ! suspended, waves
            if (dqc == 0.0_dp .and. dqw==0.0_dp) then
               cycle
            end if
            !
            dq = dqc + dqw
            ...
            dqc = dqc * fixf
            dqw = dqw * fixf
            susslopeflux(lf, l) = dqc + dqw
            e_ssn(lf, lsus) = e_ssn(lf, lsus) + dqc
            e_sswn(lf, lsus) = e_sswn(lf, lsus) + dqw
         end do
      end do
