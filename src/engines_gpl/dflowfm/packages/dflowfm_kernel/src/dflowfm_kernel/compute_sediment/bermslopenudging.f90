   ...
   do L = 1, lnx
      ...
      do lsd = 1, lsedtot
         ...
         if (bermslopeindexsus(L) .and. sus /= 0.0 .and. lsd <= lsed) then
            ...
            call getfracfixfac(L, k1, k2, lsd, e_ssn(L, lsd), fixf)
...
subroutine getfracfixfac(L, k1, k2, lsd, transp, fixf)
   use m_fm_erosed
   use m_flow, only: hu, epshu
   use m_flowgeom
