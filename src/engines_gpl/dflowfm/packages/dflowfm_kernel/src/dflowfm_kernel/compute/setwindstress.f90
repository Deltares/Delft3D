...
contains
   ...
   subroutine setwindstress()
      ...
      use m_flow, only: write_map_output, rho_water_in_wind_stress, RHO_MEAN, wdsu, ktop, rho, wdsu_x, wdsu_y, rhomean, &
      ...
      if (jawindstressgiven > 0) then
         ...
         if (jastresstowind == 0) then ! stress directly
            if (write_map_output%wind > 0) then
               wx = 0.0_dp
               wy = 0.0_dp
            end if
            do L = 1, lnx
      ...
      if (jawindstressgiven == 0 .or. jastresstowind == 1) then
         ...
         do L = 1, lnx
            if (wx(L) /= 0.0_dp .or. wy(L) /= 0.0_dp .or. relativewind > 0) then ! only if some wind
               ...
               tuwi = roa * cdw * uwi
               if (write_map_output%windstress > 0) then
                  wdsu_x(L) = tuwi * wxL
                  wdsu_y(L) = tuwi * wyL
               end if
               if (kmx > 0) then
