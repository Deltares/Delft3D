...
contains
   ...
   module subroutine add_lateral_load_and_sink(transport_load, transport_sink, cell_volume, dtol)
      ...
      real(kind=dp), dimension(:, :), intent(inout) :: transport_load !< Load being transported into domain. Sign-convention: positive means load being transported into model.
      real(kind=dp), dimension(:, :), intent(inout) :: transport_sink !< Load being transported out. Sign-convention: positive means load being transported out.
      real(kind=dp), dimension(:), intent(in) :: cell_volume !< Volume of water in computational cells [m3]
         ...
         do i_const = 1, numconst
            do i_lateral = 1, numlatsg
               do k1 = n1latsg(i_lateral), n2latsg(i_lateral)
                  ...
                  ! The sign-convention for sink: positive means flux going out of model. Hence, negative sign is introduced here:
