...
contains
   ...
   subroutine setucxucyucxuucyu()
      ...
      use m_flowparameters, only: jabarrieradvection, jasedtrails, write_map_output, Perot_type, NOT_DEFINED, changeVelocityAtStructures, jacstbnd, jaZerozbndinflowadvection, JaZlayercenterbedvel, PEROT_VOLUME_BASED, epshs, limtypmom, jarhoxu
      use m_sferic
      use m_get_Lbot_Ltop
      ...
      if (Perot_type /= NOT_DEFINED) then
         ...
         make2dh = (kmx < 1) .or. (kmx > 0 .and. (jasedtrails > 0 .or. write_map_output%ucmag > 0 .or. write_map_output%ucvec > 0))
