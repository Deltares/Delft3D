module m_atmospheric_stability
   ...
   real(kind=dp), parameter :: CONST_KAPPA = 0.4_dp ! von Kármán constant [-]
   ...
   type :: t_options
      logical :: include_free_convection = .false.  !< Include free convection.
      logical :: include_fqsat = .false.            !< Include reduction factor of saturation specific humidity due to salt.
      logical :: include_monin_obukhov = .true.     !< Include atmospheric stability in computing scaling parameters.
   end type t_options
   ...
   type :: t_scales
      ...
      real(kind=dp) :: w_star = 0.0_dp                 !< Gustiness velocity [m/s].
...
contains
   ...
   subroutine compute_scaling_parameters(wind_velocity_u, wind_velocity_v, air_temperature, dew_point_temperature, &
                                  air_pressure, charnock, surface_temperature, result, options)
      ...
      type(t_scales), intent(out) :: result                !< Computed scaling outputs.
      type(t_options), intent(in), optional :: options     !< Optional model switches.

      type(t_options) :: runtime_options
      ...
      real(kind=dp) :: u_star_new, t_star_new, q_star_new
      ...
      if (runtime_options%include_fqsat) then
      ...
      temperature_pfactor = 0.04_dp * log(HEIGHT_WIND_VELOCITY / HEIGHT_TEMPERATURE) / CONST_KAPPA
      temperature_at_wind_height = (air_temperature - temperature_pfactor*surface_temperature) / (1.0_dp - temperature_pfactor)
      humidity_pfactor = 0.04_dp * log(HEIGHT_WIND_VELOCITY / HEIGHT_HUMIDITY) / CONST_KAPPA
      humidity_at_wind_height = (air_humidity - humidity_pfactor*surface_humidity) / (1.0_dp - humidity_pfactor)

      if (runtime_options%include_free_convection) then
      ...
      do iteration = 1, MAXIMUM_ITERATION
         ...
         if (runtime_options%include_monin_obukhov) then
             ...
             if (abs(HEIGHT_WIND_VELOCITY - HEIGHT_TEMPERATURE) > 0.01_dp) then
                ...
                temperature_at_wind_height = air_temperature + (t_star / CONST_KAPPA) * heat_profile_correction
                delta_temperature = temperature_at_wind_height - surface_temperature
             end if
             ...
             if (abs(HEIGHT_WIND_VELOCITY - HEIGHT_HUMIDITY) > 0.01_dp) then
                ...
                humidity_at_wind_height = air_humidity + (q_star / CONST_KAPPA) * humidity_profile_correction
                delta_specific_humidity = humidity_at_wind_height - surface_humidity
             end if
         ...
         u_star_new = CONST_KAPPA * delta_wind_speed / log_denominator_momentum
         t_star_new = CONST_KAPPA * delta_temperature / log_denominator_heat
         q_star_new = CONST_KAPPA * delta_specific_humidity / log_denominator_humidity

         convergence_error = sqrt((u_star_new - u_star)**2 + (t_star_new - t_star)**2 + (q_star_new - q_star)**2)
   ...
   end subroutine compute_scaling_parameters
   ...
   subroutine compute_scales_and_fluxes(wind_velocity_u, wind_velocity_v, air_temperature, dew_point_temperature, &
      ...
      do index = 1, number_of_elements
         call compute_scaling_parameters(wind_velocity_u(index), wind_velocity_v(index), air_temperature(index), &
                                         dew_point_temperature(index), air_pressure(index), charnock(index), &
                                         surface_temperature(index), scaling_parameter, options)
   ...
   pure subroutine compute_roughness_lengths(u_star, charnock, z0_momentum, z0_heat, z0_humidity)
      ...
      z0_momentum = ALPHA_M*CONST_NU_AIR/u_star + charnock*u_star*u_star/CONST_GRAVITY
      z0_heat = ALPHA_H*CONST_NU_AIR/u_star
      z0_humidity = ALPHA_Q*CONST_NU_AIR/u_star
   ...
   pure function stability_profile_heat_humidity(stability_parameter) result(stability_correction)
      ...
      psi_unstable = 2.0_dp * log((1.0_dp + unstable_factor*unstable_factor) / 2.0_dp)
      ...
      if (clipped_zeta < 0.0_dp) then
         ...
         stability_correction = psi_stable
      else
