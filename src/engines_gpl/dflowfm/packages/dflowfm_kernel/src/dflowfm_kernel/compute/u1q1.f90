...
contains
   ...
   subroutine u1q1()
      ...
      use m_flowtimes, only: ti_waq, dts, autotimestep, AUTOTS_3D_HOR_OUT, AUTOTS_3D_HOR_INOUT
...
!                        diferences in to node, negative extra discharges in the vertical
         ...
         if (autotimestep == AUTOTS_3D_HOR_OUT .or. autotimestep == AUTOTS_3D_HOR_INOUT) then ! 2D timestep
            squ2d = squ
            if (autotimestep == AUTOTS_3D_HOR_INOUT) then
               squ2d = squ2d + sqi
            end if
         end if
