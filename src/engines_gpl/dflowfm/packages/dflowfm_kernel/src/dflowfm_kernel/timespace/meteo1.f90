...
999   continue
   ...
   subroutine meteo_tidepotential(jul0, TIME, dstart, dstop, eps)
      use m_sferic
      use m_flowparameters, only: jatidep, jaselfal, write_map_output
      use m_partitioninfo
      use m_flow
      use m_flowgeom
...
!        for output only
            if (write_map_output%tidep > 0 .and. Np > 1) then ! store SAL potential seperately
...
contains
   ...
   function fm_ext_force_name_to_ec_item(trname, sfname, waqinput, constituent_name, qidname, &
      ...
      case ('hrms', 'wavesignificantheight')
         itemPtr1 => item_hrms
         dataPtr1 => hwavcom
         write_map_output%wav_hwav = 1
      case ('tp', 'tps', 'rtp', 'waveperiod')
         itemPtr1 => item_tp
         dataPtr1 => twavcom
         write_map_output%wav_twav = 1
      case ('dir', 'wavedirection')
         itemPtr1 => item_dir
         dataPtr1 => phiwav
         write_map_output%wav_phiwav = 1
