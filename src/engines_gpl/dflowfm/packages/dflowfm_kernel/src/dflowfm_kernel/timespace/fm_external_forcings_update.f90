...
submodule(fm_external_forcings) fm_external_forcings_update
   ...
   use m_meteo, only: ec_gettimespacevalue, ecgetvalues, twav, success, air_pressure, pavbnd, ja_airdensity, item_air_density, &
                      ...
                      item_lateraldischarge, npumpswithlevels, num_source_sink, item_discharge_salinity_temperature_sorsin, source_sink_discharge, &
...
contains
   ...
   module subroutine set_external_forcings(time_in_seconds, initialization, iresult)
      ...
      integer :: i_const
      real(kind=dp), dimension(:), pointer :: source_sink_discharge_1d !< 1D pointer view of 2D source_sink_discharge array
      ...
      if (num_source_sink > 0) then
         ! Create 1D pointer view of 2D source_sink_discharge array to pass to ec_gettimespacevalue
         ! This avoids copying while satisfying the 1D array interface requirement
         source_sink_discharge_1d(1:size(source_sink_discharge)) => source_sink_discharge
