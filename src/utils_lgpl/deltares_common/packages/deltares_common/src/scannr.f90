...
contains
   ...
   subroutine scannr(string, stapos, endpos, nrflds, itype, ifield, rfield, cfield, lenchr, maxfld, lconvu, lconv1, lconv2)
      ! Arguments
      integer, intent(in) :: endpos !< End position of scan
      integer, intent(in) :: maxfld !< Size of buffers (max number of sub-fields).
      integer, intent(out) :: nrflds !< Number of found sub-fields
                                     !! Error values:
      ...
      logical, intent(in) :: lconv1 !<  Logical to determine if strings delimited by ' must be converted to integer or real if possible.
      logical, intent(in) :: lconv2 !<  Logical to determine if strings delimited by " must be converted to integer or real if possible.
      ...
      character(len=1) :: quote1
      character(len=1) :: quote2
      character(len=1) :: tab
      ...
      do chrpos = stapos, endpos + 1
         ...
         if (chrpos > endpos) then
         ...
         else if (.not. lqstr) then
            if (string(chrpos:chrpos) == quote1 .or. string(chrpos:chrpos) == quote2) then
            ...
            else if (string(chrpos:chrpos) == blank .or. string(chrpos:chrpos) == tab) then
               if (stachr > 0) then

                  ! End of unquoted sub-string found
                  endchr = chrpos - 1
               end if
            else if (string(chrpos:chrpos) /= blank .and. string(chrpos:chrpos) /= tab) then
               if (stachr == -1) then
                  ! Start of unquoted sub-string found
                  stachr = chrpos
               end if
            end if
         else if (string(chrpos:chrpos) == quote) then
         ...
         if (stachr > 0 .and. endchr > 0) then
            ...
            if (nbrchr > 0) then
               ...
               if (lqstr) then
                  if ((quote == '''' .and. lconv1) .or. (quote == '"' .and. lconv2)) then
                     lconv = .true.
                  else
                     lconv = .false.
                  end if
