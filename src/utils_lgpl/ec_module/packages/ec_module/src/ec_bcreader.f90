                  ...
                  case (BT_FORCING)
                     if (jaheader) then
                        ...
                        if (posfs > 0) then
                           ...
                           end if ! Right quantity
                           jaheader = .false. ! No, we are NOT reading a header
                        end if ! switch to datamode
                     end if ! in header mode (data lines are ignored)
                  case (BT_GENERAL)
                     posfs = index(rec, '=')
                     if (posfs > 0) then
