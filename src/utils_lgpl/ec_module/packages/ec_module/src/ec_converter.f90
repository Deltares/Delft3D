...
10       continue
   ...
   function ecConverterNetcdf(connection, timesteps) result(success)
      ...
      real(dp), dimension(:), pointer :: targetValues
      real(dp), dimension(:), allocatable :: source_sink_z_bot
      real(dp) :: ztgt
      real(dp) :: PI, phi, xtmp
         ...
         do i = 1, connection%nSourceItems
            ...
            if (sourceElementSet%ofType == elmSetType_samples) then
               ...
               if (n_layers > 0 .and. associated(targetElementSet%z) .and. associated(sourceElementSet%z)) then
                  allocate (source_sink_z_bot(n_layers))
                  if (issparse == 1) then
                  ...
                  do j = 1, n_points
                     ...
                     if (mp > 0 .and. np > 0) then
                        ...
                        source_sink_z_bot = (a_s * sourceElementSet%z + b_s - b_t) / a_t

                        ! initialize upper layer kp
                        kp = 2

                        ! dkp: increase direction of (scaled) source z-coordinate source_sink_z_bot, i.e. zrsc(kp) > source_sink_z_bot(kp-dkp)
                        if (source_sink_z_bot(2) - source_sink_z_bot(1) > 0) then
                           dkp = 1
                        else
                           dkp = -1
                        ...
                        do k = kbot, ktop
                           ...
                           ! get search direction in source_sink_z_bot
                           if (dkp * (ztgt - source_sink_z_bot(kp)) > 0) then
                              k_inc = 1
                           else
                              k_inc = -1
                           end if
                           ...
                           do while ((source_sink_z_bot(kp - dkp) > ztgt) .or. (source_sink_z_bot(kp) <= ztgt))
                              kp = kp + k_inc
                           ...
                           if (jamissing > 0) then
                              ...
                              wb = (source_sink_z_bot(kp) - ztgt) / (source_sink_z_bot(kp) - source_sink_z_bot(kp - dkp))
                  ...
                  if (allocated(source_sink_z_bot)) deallocate (source_sink_z_bot)
               else
                  if (issparse == 1) then
                     Ndatasize = ia(n_rows + 1) - 1
