...
!==============================================================================================================
   ...
   function ecProviderCreateNetcdfItems(instancePtr, fileReaderPtr, quantityName, varname) result(success)
      ...
      real(dp) :: fgd_fillvalue !< _FillValue for first coordinate
      real(dp) :: sgd_fillvalue !< _FillValue for second coordinate
      ...
      do i = 1, expectedLength
         ...
         if (grid_type == ec_undef_int) then
            ...
            if (ndims == 2) then
               ...
               ierror = nf90_get_att(fileReaderPtr%fileHandle, fgd_id, '_FillValue', fgd_fillvalue)
               if (ierror /= nf90_noerr) fgd_fillvalue = ieee_value(0.0_dp, ieee_quiet_nan)
               ierror = nf90_get_att(fileReaderPtr%fileHandle, sgd_id, '_FillValue', sgd_fillvalue)
               if (ierror /= nf90_noerr) sgd_fillvalue = ieee_value(0.0_dp, ieee_quiet_nan)
               
               ierror = nf90_get_var(fileReaderPtr%fileHandle, fgd_id, fgd_data, start=(/1, 1/), count=crd_dimlen(1:2, 1))
               where (fgd_data == fgd_fillvalue) fgd_data = nan_value
               ierror = nf90_get_var(fileReaderPtr%fileHandle, sgd_id, sgd_data, start=(/1, 1/), count=crd_dimlen(1:2, 2))
               where (sgd_data == sgd_fillvalue) sgd_data = nan_value
               ...
               where (fgd_data_1d(1:crd_dimlen(1, 1)) == fgd_fillvalue) fgd_data_1d(1:crd_dimlen(1, 1)) = nan_value
               ierror = nf90_get_var(fileReaderPtr%fileHandle, sgd_id, sgd_data_1d(1:crd_dimlen(1, 2)), start=(/1/), count=(/crd_dimlen(1, 2)/))
               where (sgd_data_1d(1:crd_dimlen(1, 2)) == sgd_fillvalue) sgd_data_1d(1:crd_dimlen(1, 2)) = nan_value
