...
function t_ug_meshgeom_destructor(meshgeom) result(ierr)
   ...
   if(associated(meshgeom%nodeidx)) deallocate(meshgeom%nodeidx)
   if(associated(meshgeom%nodeidx_inverse)) deallocate(meshgeom%nodeidx_inverse)
   if(associated(meshgeom%nodeoffsets)) deallocate(meshgeom%nodeoffsets)
   if(associated(meshgeom%edgebranchidx)) deallocate(meshgeom%edgebranchidx)
   if(associated(meshgeom%edgeoffsets)) deallocate(meshgeom%edgeoffsets)
   ...
   if(associated(meshgeom%branchlongnames)) deallocate(meshgeom%branchlongnames)
   if(associated(meshgeom%nnodeids)) deallocate(meshgeom%nnodeids)
   if(associated(meshgeom%nnodelongnames)) deallocate(meshgeom%nnodelongnames)
   ...
   meshgeom%nodeidx => null()
   meshgeom%nodeidx_inverse => null()
   meshgeom%nodeoffsets => null()
   meshgeom%edgebranchidx => null()
   meshgeom%edgeoffsets => null()
   ...
   meshgeom%branchids         => null()
   meshgeom%branchlongnames   => null()
   meshgeom%nnodeids         => null()
   meshgeom%nnodelongnames   => null()

   meshgeom = t_ug_meshgeom()
