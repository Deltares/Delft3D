...
internal static class NativeInterop
{
    ...
    {
        var result = new double[(int)count];
        if (ptr != IntPtr.Zero)
        {
    ...
    private static TManaged[] MarshalStructArray<TNative, TManaged>(IntPtr ptr, ulong count, Func<TNative, TManaged> convert)
        where TNative : struct
    {
        var result = new TManaged[(int)count];
        int structSize = Marshal.SizeOf(typeof(TNative));

        for (var i = 0; i < (int)count; i++)
        {
            var elementPtr = new IntPtr(ptr.ToInt64() + (i * structSize));
            var native = (TNative)Marshal.PtrToStructure(elementPtr, typeof(TNative))!;
