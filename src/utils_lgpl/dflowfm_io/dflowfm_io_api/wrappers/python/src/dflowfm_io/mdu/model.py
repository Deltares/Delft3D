...
Hand-maintained (like base/bindings.py): the C accessor set is small and rarely changes. When the C
ABI gains or changes an accessor, edit this file to match dflowfm_io_api.h.
"""
...
class MduModel:
    ...
    def get_string_enum(self, key: str) -> str:
        value = ctypes.c_char_p()
        check_result(lib.mdu_get_string_enum(self._ref.handle, key.encode("utf-8"), ctypes.byref(value)))
        return value.value.decode("utf-8")

    def get_int_enum(self, key: str) -> int:
        value = ctypes.c_int32()
        check_result(lib.mdu_get_int_enum(self._ref.handle, key.encode("utf-8"), ctypes.byref(value)))
        return value.value
    ...
    def set_int_enum(self, key: str, value: int) -> None:
        check_result(lib.mdu_set_int_enum(self._ref.handle, key.encode("utf-8"), ctypes.c_int32(value)))
