...
class MduDocument:
    ...
    def __del__(self):
        if hasattr(self, "_ref") and self._ref.handle:
            _lib.mdu_destroy(ctypes.byref(self._ref.handle))
    ...
    def load_from_file(self, filename: str) -> None:
        _check_result(_lib.mdu_load_from_file(self._ref.handle, filename.encode("utf-8")))

    def load_from_lines(self, data: list[str]) -> None:
        encoded = "\n".join(data).encode("utf-8")
        _check_result(_lib.mdu_load_from_string(self._ref.handle, encoded, ctypes.c_uint64(len(encoded))))

    def save_to_file(self, filename: str) -> None:
        _check_result(_lib.mdu_save_to_file(self._ref.handle, filename.encode("utf-8")))

    def save_to_lines(self) -> list[str]:
        string_out = ctypes.c_char_p()
        _check_result(_lib.mdu_save_to_string(self._ref.handle, ctypes.byref(string_out)))
...
class MduReport:
    ...
    def get_issues(self) -> list[Issue]:
        ...
        _check_result(_lib.mdu_get_issue_list(self._ref.handle, ctypes.byref(array_out), ctypes.byref(size_out)))
        issues = []
...
class MduModel:
    ...
    def get_bool(self, key: str) -> bool:
        value = ctypes.c_int32()
        _check_result(_lib.mdu_get_bool(self._ref.handle, key.encode("utf-8"), ctypes.byref(value)))
        return value.value != 0

    def get_double(self, key: str) -> float:
        value = ctypes.c_double()
        _check_result(_lib.mdu_get_double(self._ref.handle, key.encode("utf-8"), ctypes.byref(value)))
        return value.value

    def get_string(self, key: str) -> str:
        string_out = ctypes.c_char_p()
        _check_result(_lib.mdu_get_string(self._ref.handle, key.encode("utf-8"), ctypes.byref(string_out)))
        return string_out.value.decode("utf-8")

    def get_path(self, key: str) -> Path:
        path_out = ctypes.c_char_p()
        _check_result(_lib.mdu_get_path(self._ref.handle, key.encode("utf-8"), ctypes.byref(path_out)))
    ...
    def get_string_list(self, key: str) -> list[str]:
        ...
        size_out = ctypes.c_uint64()
        _check_result(_lib.mdu_get_string_list(self._ref.handle, key.encode("utf-8"), ctypes.byref(array_out), ctypes.byref(size_out)))
        return [array_out[i].decode("utf-8") for i in range(size_out.value)]

    def get_path_list(self, key: str) -> list[Path]:
        array_out = ctypes.POINTER(ctypes.c_char_p)()
        size_out = ctypes.c_uint64()
        _check_result(_lib.mdu_get_path_list(self._ref.handle, key.encode("utf-8"), ctypes.byref(array_out), ctypes.byref(size_out)))
    ...
    def get_double_list(self, key: str) -> list[float]:
        ...
        size_out = ctypes.c_uint64()
        _check_result(_lib.mdu_get_double_list(self._ref.handle, key.encode("utf-8"), ctypes.byref(array_out), ctypes.byref(size_out)))
        return [array_out[i] for i in range(size_out.value)]

    def set_int(self, key: str, value: int) -> None:
        _check_result(_lib.mdu_set_int(self._ref.handle, key.encode("utf-8"), ctypes.c_int32(value)))

    def set_bool(self, key: str, value: bool) -> None:
        _check_result(_lib.mdu_set_bool(self._ref.handle, key.encode("utf-8"), ctypes.c_int32(1 if value else 0)))

    def set_double(self, key: str, value: float) -> None:
        _check_result(_lib.mdu_set_double(self._ref.handle, key.encode("utf-8"), ctypes.c_double(value)))

    def set_string(self, key: str, value: str) -> None:
        _check_result(_lib.mdu_set_string(self._ref.handle, key.encode("utf-8"), value.encode("utf-8")))

    def set_path(self, key: str, value: Path | str) -> None:
        _check_result(_lib.mdu_set_path(self._ref.handle, key.encode("utf-8"), str(value).encode("utf-8")))
    ...
    def set_string_list(self, key: str, values: list[str]) -> None:
        ...
        _check_result(_lib.mdu_set_string_list(self._ref.handle, key.encode("utf-8"), arr, ctypes.c_uint64(len(encoded))))

    def set_path_list(self, key: str, values: list[Path | str]) -> None:
        encoded = [str(v).encode("utf-8") for v in values]
        arr = (ctypes.c_char_p * len(encoded))(*encoded)
        _check_result(_lib.mdu_set_path_list(self._ref.handle, key.encode("utf-8"), arr, ctypes.c_uint64(len(encoded))))

    def set_double_list(self, key: str, values: list[float]) -> None:
        arr = (ctypes.c_double * len(values))(*values)
        _check_result(_lib.mdu_set_double_list(self._ref.handle, key.encode("utf-8"), arr, ctypes.c_uint64(len(values))))