...
function show_help {
    cat - <<EOF
Usage: $0 -a <apptainer-image> -r <s3-path-prefix> -o <s3-path-prefix> [-m <model-path>] [-f <comma-separated list>]
...
PARSED_OPTIONS=$(getopt -o 'a:c:r:m:f:' -l 'apptainer:,current-prefix:,reference-prefix:,model-path:,model-filter:' -- "$@")
...
mkdir -p "${LOG_DIR}/models" "${VAHOME}/${MODEL_PATH}"
...
srun --nodes=1 --ntasks=1 --cpus-per-task=16 --partition=16vcpu_spot \
    ...
    docker run --rm --volume="${HOME}/.aws:/root/.aws:ro" --volume="${VAHOME}/${MODEL_PATH}:/data" \
    docker.io/amazon/aws-cli:2.22.7 \
    --profile=verschilanalyse --endpoint-url=https://s3.deltares.nl \
    s3 sync --delete --no-progress "${BUCKET}/${MODEL_PATH}/" /data
...
JOB_IDS=()
SUBMIT_SCRIPTS=$(find "${VAHOME}/${MODEL_PATH}" -type f -name submit_apptainer_h7.sh -iregex "$MODEL_REGEX")
