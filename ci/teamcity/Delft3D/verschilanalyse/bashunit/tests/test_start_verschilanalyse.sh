...
function set_up_before_script() {
  ...
  declare -g TMP_DIR=$(mktemp "$TMP_DIR")
  declare -g BUILDS_DIR="$TMP_DIR/builds"
  declare -g VAHOME="${BUILDS_DIR}/${BUILD_ID:-latest}"
...
# # ------ parse_args tests ------ #
# function test_parse_args_optional_empty_args() {
#   parse_args \
#     -a "$APPTAINER" \
#     -c "$CURRENT_PREFIX" \
#     -r "$REFERENCE_PREFIX" \
...
# # function test_prepare_modules_calls_module_commands() {
# #   prepare_modules
# #   assert_have_been_called_times 2
...
#   assert_matches "--model-dir ${VAHOME}/${MODELS_PATH}/model1" "$LOG"
#   assert_exit_code 0
# }
...
# function test_archive_and_upload_output_checks_sbatch_called_with_excluding_job_id() {
#   submit_jobs
#   archive_and_upload_output
...
# function test_archive_and_upload_output_checks_sbatch_called_with_including_job_id() {
#   JOB_ID=("12345")
...
function test_run_verschillentool_with_defaults() {
...
function test_run_verschillentool_check_called_with_defaults() {
  ...
  assert_have_been_called sbatch
  assert_have_been_called_with "--parsable --output=/builds/latest/logs/va-run-verschillentool-%j.out --dependency=afterany:${DOWNLOAD_REFS_JOB_ID}:${UPLOAD_OUTPUT_JOB_ID} ./jobs/run_verschillentool.sh" sbatch
  assert_have_been_called_times 3 sbatch
...
function test_run_verschillentool_check_called_with_job_id() {
  submit_jobs
  local DOWNLOAD_REFS_JOB_ID
  local UPLOAD_OUTPUT_JOB_ID
  local JOB_ID=("12345")
  ...
  assert_have_been_called sbatch
  assert_have_been_called_with "--parsable --output=/builds/latest/logs/va-run-verschillentool-%j.out --dependency=afterany:${DOWNLOAD_REFS_JOB_ID}:12345 ./jobs/run_verschillentool.sh" sbatch
  assert_have_been_called_times 3 sbatch
  assert_exit_code 0
}
