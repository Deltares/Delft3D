...
object WindowsTest : BuildType({
    ...
    artifactRules = """
            ...
            +:test\deltares_testbench\data\cases\**\*_work\**\*.pdf => pdf
            +:test\deltares_testbench\data\cases\**\*_work\**\*.dia => logging
            +:test\deltares_testbench\data\cases\**\*_work\**\*.log => logging
    ...
    steps {
        ...
        python {
            ...
            dockerPull = true
            dockerRunParameters = "--memory %teamcity.agent.hardware.memorySizeMb%m --cpus %teamcity.agent.hardware.cpuCount% -v \"C:\\dvc-cache\\delft3d:C:\\dvc-cache\""
        }
        script {
            name = "Copy cases"
            executionMode = BuildStep.ExecutionMode.RUN_ON_FAILURE
