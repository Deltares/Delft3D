...
def _timeouts_by_name(platform: str) -> dict[str, float]:
    catalog = load_case_timeouts(CSV_PATH, CONFIGS_ROOT)
    return {case.name: case.timeout_s for case in catalog if case.platform == platform}
...
def test_load_case_timeouts_skips_inactive_and_missing(caplog: pytest.LogCaptureFixture) -> None:
    catalog = load_case_timeouts(CSV_PATH, CONFIGS_ROOT)
    names = {(case.platform, case.name) for case in catalog}
...
def test_conflicting_timeouts_use_minimum() -> None:
    catalog = load_case_timeouts(CSV_PATH, CONFIGS_ROOT)
    conflict = next(case for case in catalog if case.platform == "linux" and case.name == "conflict_case")

    assert conflict.timeout_s == 120.0
    assert conflict.timeout_conflict is True
    assert conflict.config_files == (
        "dimr/sample_lnx64.xml",
        "dimr/sample_lnx64_conflict.xml",
    )


def test_platforms_are_split() -> None:
    linux = _timeouts_by_name("linux")
    windows = _timeouts_by_name("windows")

    assert "windows_only" not in linux
